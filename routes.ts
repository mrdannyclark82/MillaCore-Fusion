...
export async function registerRoutes(app: Express): Promise<HttpServer> {
  ...
  app.post('/api/chat', async (req, res) => {
    ...
    try {
      ...
      // Check for /nf/ bypass flag to skip function calls
      let bypassFunctionCalls = false;
      if (message.trim().startsWith('/nf/')) {
        bypassFunctionCalls = true;
        message = message.trim().substring(4).trim(); // Remove /nf/ prefix
        console.log('🚫 Function call bypass enabled for this message');
      }
      ...
      if (!bypassFunctionCalls) {
        ...
        console.log(`🎯 Command trigger: service="${parsedCommand.service}", action="${parsedCommand.action}"`);
        ...
        console.log('🚫 Skipping function call parsing due to /nf/ bypass flag');
      }
...
*settles back into devoted spouse mode* What else can I help you with today, love? 💜`,
  ...
  // Check for code generation requests first (skip if bypassed)
  if (!bypassFunctionCalls) {
    const codeRequest = extractCodeRequest(userMessage);
    if (codeRequest) {
      console.log(`🎯 Trigger: Code generation request detected - "${codeRequest.prompt}" (language: ${codeRequest.language || 'auto'})`);
      try {
